...
def register_supervisor_tools(mcp: Any, ctx: Any) -> None:
    ...
    def recommend_workflow(
        ...
        return {
            ...
            "all_prompts": [
                "equity_deep_dive", "sector_comparison", "risk_report",
                "volatility_deep_dive", "regime_detection", "pairs_trading",
                "portfolio_construction", "ml_alpha_research", "option_pricing",
            ],
        }
    ...
    def module_guide(module: str) -> dict[str, Any]:
        ...
        guides = {
            "risk": {
                ...
                "key_tools": ["risk_metrics", "var_analysis", "stress_test", "beta_analysis", "factor_analysis", "crisis_drawdowns", "portfolio_risk"],
                "when_to_use": "After computing returns, to assess how much risk you're taking and where it comes from",
                "feeds_into": ["opt (risk-aware optimization)", "backtest (risk-adjusted metrics)", "viz (risk dashboards)"],
                "feeds_from": ["vol (GARCH conditional vol for VaR)", "regimes (regime-conditional risk)", "stats (correlation for portfolio risk)"],
                "functions": 95,
            },
            "vol": {
                "description": "Volatility modeling and forecasting",
                "key_tools": ["fit_garch", "forecast_volatility", "model_selection", "news_impact_curve", "realized_volatility"],
                "when_to_use": "To understand volatility dynamics — is vol high/low, rising/falling, symmetric/asymmetric?",
                "feeds_into": ["risk (GARCH VaR)", "backtest (vol-based position sizing)", "regimes (vol regime detection)"],
                "feeds_from": ["data (price series)", "stats (return statistics)"],
                "functions": 28,
            },
            "regimes": {
                "description": "Market regime detection and regime-aware investing",
                "key_tools": ["detect_regimes", "regime_statistics", "select_n_states", "rolling_regime_probability"],
                "when_to_use": "To identify current market state (bull/bear, high/low vol) and adapt strategy accordingly",
                "feeds_into": ["opt (regime-conditional weights)", "backtest (regime-filtered signals)", "risk (regime-specific metrics)"],
            ...
            "ta": {
                ...
                "key_tools": ["compute_indicator", "list_indicators", "multi_indicator", "scan_signals"],
                "when_to_use": "For generating trading signals, identifying trends, measuring momentum, detecting patterns",
                "feeds_into": ["ml (TA features)", "backtest (signal-based strategies)", "viz (chart overlays)"],
                "categories": ["momentum", "overlap", "volume", "trend", "volatility", "patterns", "signals", "statistics", "cycles", "fibonacci", "support_resistance", "breadth", "performance", "smoothing", "exotic", "candles", "price_action", "custom"],
                "functions": 265,
            },
            "stats": {
                "description": "Statistical analysis — regression, correlation, distributions, tests",
                "key_tools": ["correlation_analysis", "regression", "distribution_fit", "stationarity_tests", "cointegration_test"],
                "when_to_use": "For understanding data properties, relationships between assets, and statistical significance",
                "feeds_into": ["risk (correlation for portfolio risk)", "ml (features)", "ts (stationarity for forecasting)"],
                "functions": 79,
            },
            "ts": {
                "description": "Time series analysis — forecasting, decomposition, changepoints",
                "key_tools": ["forecast", "decompose", "stationarity_test", "changepoint_detect", "anomaly_detect"],
                "when_to_use": "For understanding time series structure and predicting future values",
                "feeds_into": ["backtest (forecast-based strategies)", "risk (vol forecasting)"],
                "functions": 52,
            },
            "opt": {
                "description": "Portfolio optimization — MVO, risk parity, Black-Litterman, HRP",
                "key_tools": ["optimize_portfolio", "efficient_frontier", "rebalance_analysis"],
                "when_to_use": "After computing returns and risk, to find optimal portfolio weights",
                "feeds_into": ["backtest (test the allocation)", "execution (trade to target weights)"],
                "feeds_from": ["risk (portfolio vol, component VaR)", "regimes (regime adjustment)", "stats (shrunk covariance)"],
                "functions": 26,
            },
            "backtest": {
                "description": "Strategy backtesting and performance evaluation",
                "key_tools": ["run_backtest", "backtest_metrics", "comprehensive_tearsheet", "walk_forward", "strategy_comparison"],
                "when_to_use": "To test whether a strategy works historically before risking capital",
                "feeds_from": ["ta (signals)", "ml (predictions)", "opt (weights)", "regimes (filters)"],
                "functions": 38,
            },
            "price": {
                "description": "Derivatives pricing — options, fixed income, stochastic models",
                "key_tools": ["price_option", "compute_greeks", "simulate_process", "yield_curve_analysis"],
            ...
            "ml": {
                ...
                "key_tools": ["build_features", "train_model", "feature_importance", "walk_forward_ml"],
                "when_to_use": "For building predictive models from financial data",
                "feeds_from": ["ta (indicator features)", "stats (statistical features)", "regimes (regime features)"],
                "feeds_into": ["backtest (ML signals)", "risk (ML-based risk)"],
                "functions": 44,
            },
            "causal": {
                "description": "Causal inference — DID, synthetic control, IV, event studies",
                "key_tools": ["granger_causality", "event_study", "diff_in_diff", "synthetic_control"],
            ...
            "bayes": {
                ...
                "key_tools": ["bayesian_sharpe", "bayesian_regression", "bayesian_changepoint", "model_comparison"],
            ...
            "microstructure": {
                ...
                "key_tools": ["liquidity_metrics", "toxicity_analysis", "market_quality", "spread_decomposition"],
            ...
            "math": {
                ...
                "key_tools": ["correlation_network", "levy_simulate", "optimal_stopping"],
